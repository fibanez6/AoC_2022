...
class CycleDetector:
    ...
    start_pointer: int = None
    end_pointer: int = None
    cycle_length: int = 0
    ...
    def check(self, ite, state):
        ...
        if state in self.history:
            if self.start_pointer is None:
                self.start_pointer = self.history[state]
                self.end_pointer = ite
                self.cycle_length = self.end_pointer - self.start_pointer
                if DEBUG: print(f'state {state} seen {self.start_pointer}')

            if self.diff_heights[self.history[state]] == self.diff_heights[self.start_pointer]:
                if self.start_pointer == self.end_pointer:
                    if DEBUG: print(f'Cycle found at {ite} with length {self.cycle_length}')
                    self.found = True
                    return
                else:
                    self.start_pointer += 1

        elif self.start_pointer is not None:
